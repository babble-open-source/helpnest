    ...
    <div className="p-4 sm:p-8">
        ...
        {collection.parent?.parent && (
          <>
            ...
            <Tooltip content={collection.parent.parent.title}>
              <Link href={`/collections/${collection.parent.parent.id}`} className="hover:text-foreground transition-colors truncate max-w-[80px] sm:max-w-[150px] lg:max-w-[220px] block">
        ...
        {collection.parent && (
          <>
            ...
            <Tooltip content={collection.parent.title}>
              <Link href={`/collections/${collection.parent.id}`} className="hover:text-foreground transition-colors truncate max-w-[80px] sm:max-w-[150px] lg:max-w-[220px] block">
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div className="min-w-0 flex-1">
          <div className="flex items-start gap-3 min-w-0">
            <span className="text-2xl mt-1 shrink-0">{collection.emoji ?? '📁'}</span>
            <div className="min-w-0 flex-1">
        ...
        <div className="my-8">
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {collection.subCollections.map((sub) => (
              ...
              >
                <Link href={`/collections/${sub.id}`} className="flex items-center gap-2 min-w-0 flex-1 group">
                  <span className="text-xl">{sub.emoji ?? '📂'}</span>
                  <div className="flex items-center gap-2 min-w-0">
                    <Tooltip content={sub.title} wrapperClassName="min-w-0 overflow-hidden">
                      <p className="text-sm font-medium text-foreground group-hover:text-primary transition-colors truncate">{sub.title}</p>
                    </Tooltip>
      ...
      {articles.length === 0 && collection.subCollections.length === 0 ? (
        <Card className="p-12 text-center">
          ...
          <form action={createArticle}>
            <input type="hidden" name="collectionId" value={collection.id} />
            <Button type="submit" variant="ghost" className="text-primary hover:text-primary hover:underline text-sm h-auto p-0 hover:bg-transparent">
              {t('createArticle')}
            </Button>
          </form>
        </Card>
