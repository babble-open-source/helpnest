  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-start gap-4 p-5">
        <Link href={`/collections/${col.id}`} className="text-2xl shrink-0 mt-0.5">
          {col.emoji ?? '📄'}
        </Link>
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <Link href={`/collections/${col.id}`} className="font-medium text-foreground hover:text-primary transition-colors">
                  {col.title}
                </Link>
                {col._count.articles > 0 && (
                  <Badge variant="secondary" className="text-[11px] bg-primary/10 text-primary hover:bg-primary/10">
                ...
                {hasSubs && (
                  <Button
                    ...
                    className="h-auto rounded-full px-2 py-0.5 text-[11px] font-medium text-muted-foreground hover:border-primary/40 hover:text-primary"
                  >
              ...
              <React.Fragment key={sub.id}>
                ...
                <div className={cn('relative flex items-center pl-10 pr-2 py-2', sub.isArchived && 'opacity-60')}>
                  ...
                  <div className="flex items-center gap-2.5 min-w-0 rounded-lg hover:bg-muted/50 px-2 py-1 -my-1 -mx-2 transition-colors">
                    <Link href={`/collections/${sub.id}`} className="text-base shrink-0">
                      {sub.emoji ?? '📂'}
                    </Link>
                    <div className="flex items-center gap-2 min-w-0">
                      <Tooltip content={sub.title} wrapperClassName="min-w-0 overflow-hidden">
                        <Link href={`/collections/${sub.id}`} className="text-sm font-medium text-foreground hover:text-primary transition-colors truncate block">
                          {sub.title}
                        </Link>
                      </Tooltip>
                      {sub._count.articles > 0 && (
                        <Badge variant="secondary" className="shrink-0 text-[11px] bg-primary/10 text-primary hover:bg-primary/10">
                      ...
                      {hasGrand && (
                        <Button
                          ...
                          className="shrink-0 w-5 h-5 text-muted-foreground hover:text-foreground"
                ...
                {hasGrand && expandedSubs.has(sub.id) && sub.subCollections.map((grand, grandIdx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2.5 min-w-0 rounded-lg hover:bg-muted/50 px-2 py-1 -my-1 -mx-2 transition-colors">
                        <Link href={`/collections/${grand.id}`} className="text-base shrink-0">
                          {grand.emoji ?? '📂'}
                        </Link>
                        <div className="flex items-center gap-2 min-w-0">
                          <Tooltip content={grand.title} wrapperClassName="min-w-0 overflow-hidden">
                            <Link href={`/collections/${grand.id}`} className="text-sm font-medium text-foreground hover:text-primary transition-colors truncate block">
                              {grand.title}
                            </Link>
                          </Tooltip>
