...
export default async function ArticlesPage(props: {
  ...
  const tabs = [
    { label: tc('all'), href: '/articles', active: !searchParams.status && !searchParams.filter },
    { label: tc('published'), href: '/articles?status=PUBLISHED', active: searchParams.status === 'PUBLISHED' },
    { label: tc('draft'), href: '/articles?status=DRAFT', active: searchParams.status === 'DRAFT' },
    { label: tc('archived'), href: '/articles?status=ARCHIVED', active: searchParams.status === 'ARCHIVED' },
    ...(aiDraftCount > 0 ? [{ label: t('aiDrafts', { count: aiDraftCount }), href: '/articles?filter=ai-drafts', active: aiDraftsFilter }] : []),
    ...(aiUpdateCount > 0 ? [{ label: t('aiUpdates', { count: aiUpdateCount }), href: '/articles?filter=ai-updates', active: aiUpdatesFilter }] : []),
  ]
  ...
  return (
    <div className="p-4 sm:p-8">
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          <h1 className="font-semibold text-2xl text-foreground">{t('articles')}</h1>
