...
export function CrawlModal({
  ...
  return (
    <Dialog open onOpenChange={(isOpen) => { if (!isOpen) onClose() }}>
      <DialogContent
        className={isWide ? 'dashboard-root sm:max-w-2xl max-h-[90vh] flex flex-col' : 'dashboard-root sm:max-w-lg max-h-[90vh] flex flex-col'}
        aria-label={t('title')}
      >
        ...
        <div className="overflow-y-auto flex-1 py-2 -mx-2 px-2">
          {state === 'idle' && (
            <IdleState
              url={url}
              onUrlChange={setUrl}
          ...
          >
            <div className="min-w-0 text-left flex items-center gap-2">
              ...
              <span className="truncate">
                {selectedCollection ? selectedCollection.title : t('autoOrganize')}
              </span>
            </div>
            {selectedId && (
              <span
            ...
            <div className="p-2 border-b">
              <Input
                ...
                onChange={(e) => {
                  ...
                  items.forEach((item) => {
                    const text = item.getAttribute('data-collection-item') ?? ''
            ...
            <div className="overflow-y-auto">
              <Button
                ...
                className={`w-full justify-start px-3 py-2 text-sm h-auto rounded-none font-normal ${
                  !selectedId ? 'text-primary font-medium' : 'text-muted-foreground'
                }`}
              >
                {t('autoOrganize')}
              </Button>
              <div className="border-t" />
              {collectionsWithPath.map((c) => (
                <Button
                  key={c.id}
                  type="button"
                  variant="ghost"
                  data-collection-item={`${c.title.toLowerCase()} ${c.path.toLowerCase()}`}
...
function IdleState({
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-1.5">
        <label htmlFor="crawl-goal" className="text-sm font-medium text-foreground">
          {t('goal')}
        </label>
        <Input
          ref={goalInputRef}
      ...
      <div className="flex flex-col gap-1.5">
        <label htmlFor="crawl-url" className="text-sm font-medium text-foreground">
          {t('pageUrl')}
        </label>
        <Input
          id="crawl-url"
      ...
      <div className="bg-muted/50 border rounded-lg px-4 py-3 text-xs text-muted-foreground leading-relaxed">
        <span className="font-medium text-foreground">{t('extensionNudge')}</span>{' '}
        {t('extensionDescription')}{' '}
        <span className="text-primary font-medium">{t('extensionComingSoon')}</span>
      </div>
      ...
      <div className="bg-card border rounded-lg divide-y max-h-64 overflow-y-auto">
        {result.articles.map((article) => (
          ...
          >
            ...
            <svg
              className="ml-3 shrink-0 text-muted-foreground group-hover:text-primary transition-colors"
              width="14"
              height="14"
              viewBox="0 0 16 16"
              fill="none"
