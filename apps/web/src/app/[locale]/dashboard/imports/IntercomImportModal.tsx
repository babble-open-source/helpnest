  ...
  return (
    <Dialog open onOpenChange={(open) => { if (!open) onClose() }}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>{t('importFrom', { source: 'Intercom' })}</DialogTitle>
          <DialogDescription className="sr-only">{t('importFrom', { source: 'Intercom' })}</DialogDescription>
        </DialogHeader>

        {step === 'form' && (
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              <Label htmlFor="intercom-token">
                {t('accessToken')} <span className="text-destructive">*</span>
              </Label>
              <Input
                id="intercom-token"
                type="password"
        ...
        {step === 'importing' && (
          <div className="flex flex-col items-center gap-4 py-8">
            ...
            <p className="text-sm font-medium text-foreground">{t('importing', { source: 'Intercom' })}</p>
