      ...
      >
        ...
        {cloudMode && open && (
          <a
            ...
            className="flex items-center gap-3 px-4 py-3 hover:bg-cream/50 transition-colors group"
          >
            {workspaceLogo ? (
              <img src={workspaceLogo} alt="" className="w-8 h-8 rounded-lg object-contain border border-border" />
            ) : (
              <div className="w-8 h-8 rounded-lg bg-ink text-cream flex items-center justify-center text-sm font-medium">
                {workspaceName[0]?.toUpperCase() ?? '?'}
              </div>
            )}
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-ink truncate">{workspaceBrandText ?? workspaceName}</p>
            </div>
