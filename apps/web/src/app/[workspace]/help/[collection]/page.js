...
export default async function CollectionPage(props) {
    ...
    if (!collection || !collection.isPublic || collection.isArchived)
        notFound();
    return (<div className="min-h-screen bg-cream">
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 sm:py-12">
        ...
        <div className="mb-10">
          ...
          {collection.description && (<p className="text-muted text-lg">{collection.description}</p>)}
        </div>

        {/* Sub-collections */}
        {collection.subCollections.length > 0 && (<section className="mb-8">
            <h2 className="text-sm font-medium text-muted uppercase tracking-wide mb-3">Subcategories</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {collection.subCollections.map((sub) => (<Link key={sub.id} href={`/${params.workspace}/help/${sub.slug}`} className="flex items-center gap-3 bg-white border border-border rounded-lg p-3 hover:border-accent transition-colors group">
        ...
        {/* Articles */}
        <section>
          {collection.articles.length === 0 ? (<div className="text-center py-12 text-muted">
              <p>No articles in this collection yet.</p>
            </div>) : (<div className="bg-white rounded-xl border border-border divide-y divide-border">
              {collection.articles.map((article) => (<Link key={article.id} href={`/${params.workspace}/help/${params.collection}/${article.slug}`} className="flex items-start gap-4 p-5 hover:bg-cream/50 transition-colors group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {article.excerpt && (<p className="text-sm text-muted mt-1 line-clamp-2">{article.excerpt}</p>)}
