...
export default async function ArticlePage(props) {
    ...
    if (!article || article.status !== 'PUBLISHED' || !article.collection.isPublic || article.collection.isArchived)
        notFound();
    ...
    return (<div className="min-h-screen bg-cream">
      ...
      <nav className="sticky top-0 z-10 bg-cream/95 backdrop-blur border-b border-border">
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between gap-2 text-sm">
          <div className="flex items-center gap-2 min-w-0">
            <WorkspaceBrandLink href={`/${params.workspace}/help`} name={workspace.name} logo={workspace.logo} brandText={brandTextRecord?.brandText ?? null} hideNameWhenLogo className="shrink-0" textClassName="text-muted hover:text-ink transition-colors"/>
            <span className="text-border">/</span>
            <Link href={`/${params.workspace}/help/${params.collection}`} className="text-muted hover:text-ink transition-colors shrink-0">
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 sm:py-12">
        <div className="flex gap-8 lg:gap-12">
          ...
          <article className="flex-1 min-w-0 max-w-2xl">
            ...
            <header className="mb-8">
              <div className="flex items-center gap-2 text-xs text-muted mb-3">
                <Link href={`/${params.workspace}/help/${params.collection}`} className="bg-cream border border-border rounded-full px-2 py-0.5 hover:border-accent transition-colors">
                  {article.collection.title}
                </Link>
                <span>·</span>
            ...
            {/* Related articles */}
            {related.length > 0 && (<section className="mt-12 pt-8 border-t border-border">
                <h2 className="font-serif text-xl text-ink mb-4">Related articles</h2>
                <div className="grid gap-3">
                  {related.map((rel) => (<Link key={rel.id} href={`/${params.workspace}/help/${params.collection}/${rel.slug}`} className="flex items-center gap-3 p-4 bg-white rounded-xl border border-border hover:border-accent transition-colors group">
            ...
            <div className="mt-12 bg-ink rounded-xl p-6 text-cream text-center">
              ...
              <Link href={`/${params.workspace}/help`} className="inline-flex items-center gap-2 bg-accent text-white px-4 py-2 rounded-lg text-sm hover:bg-accent/90 transition-colors">
                Browse all articles
              </Link>
            </div>
          </article>
          ...
          {/* Sidebar TOC */}
          {headings.length > 0 && (<aside className="hidden lg:block w-56 shrink-0">
              <div className="sticky top-20">
                <p className="text-xs font-medium text-muted uppercase tracking-wide mb-3">On this page</p>
                <nav className="space-y-1">
                  {headings.map((h) => (<a key={h.id} href={`#${h.id}`} className={`block text-sm text-muted hover:text-ink transition-colors py-0.5 ${h.level === 2 ? 'pl-0' : h.level === 3 ? 'pl-3' : 'pl-0'}`}>
                      {h.text}
                    </a>))}
                </nav>
              </div>
