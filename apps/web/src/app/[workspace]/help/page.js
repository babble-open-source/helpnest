...
export default async function HelpCenterHome(props) {
    ...
    return (<div className="min-h-screen bg-cream">
      ...
      <nav className="sticky top-0 z-10 bg-cream/95 backdrop-blur border-b border-border">
        <div className="max-w-4xl mx-auto px-4 h-14 flex items-center justify-between">
          <WorkspaceBrandLink href={`/${params.workspace}/help`} name={workspace.name} logo={workspace.logo} brandText={brandTextRecord?.brandText ?? null} hideNameWhenLogo textClassName="font-serif text-xl text-ink"/>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 sm:py-12">
        ...
        <section className="mb-16">
          ...
          {workspace.collections.length === 0 ? (<p className="text-muted">No collections yet.</p>) : (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {workspace.collections.map((col) => (<Link key={col.id} href={`/${params.workspace}/help/${col.slug}`} className="group bg-white rounded-xl border border-border p-5 hover:border-accent hover:shadow-sm transition-all">
                  ...
                  {col.description && (<p className="text-sm text-muted line-clamp-2 mb-3">{col.description}</p>)}
        ...
        {/* Popular articles */}
        {workspace.articles.length > 0 && (<section className="mb-16">
            <h2 className="font-serif text-2xl text-ink mb-6">Popular articles</h2>
            <div className="bg-white rounded-xl border border-border divide-y divide-border">
              {workspace.articles.map((article) => (<Link key={article.id} href={`/${params.workspace}/help/${article.collection.slug}/${article.slug}`} className="flex items-start gap-4 p-4 hover:bg-cream/50 transition-colors group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {article.excerpt && (<p className="text-sm text-muted mt-0.5 line-clamp-1">{article.excerpt}</p>)}
                  </div>
