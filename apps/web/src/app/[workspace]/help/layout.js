...
const getWorkspaceHelpBranding = cache(async (slug) => {
...
export async function generateMetadata(props) {
    ...
    const description = state.workspace.metaDescription?.trim() || `Support docs, guides, and answers for ${state.workspace.name}.`;
 ...
 */
export default async function HelpCenterLayout({ children, ...props }) {
    const params = await props.params;
    const state = await getWorkspaceHelpBranding(params.workspace);
    if (!state)
        notFound();
