...
async function fetchInvite(token) {
    ...
    try {
        ...
        if (res.status === 410) {
            ...
            if (body.error?.includes('accepted'))
                return 'accepted';
            return 'expired';
        }
        if (!res.ok)
...
export default async function InvitePage(props) {
    ...
    if (invite === 'accepted') {
        ...
        <div className="w-full max-w-sm px-6 text-center">
          ...
          <Link href="/login" className="inline-block bg-ink text-cream px-4 py-2 rounded-lg text-sm font-medium hover:bg-ink/90 transition-colors">
            Sign in
          </Link>
        </div>
      </main>);
