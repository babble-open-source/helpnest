...
const convRateBuckets = new Map();
function getClientIp(request) {
    return (request.headers.get('x-forwarded-for')?.split(',')[0]?.trim() ||
        request.headers.get('x-real-ip')?.trim() ||
        'unknown');
}
async function checkConvRateLimit(key) {
    if (redis) {
        try {
            ...
            if (count === 1)
                await redis.pexpire(redisKey, CONV_RATE_LIMIT_WINDOW_MS * 2);
    ...
    if (convRateBuckets.size > 10_000) {
        for (const [k, b] of convRateBuckets)
            if (b.resetAt <= now)
                convRateBuckets.delete(k);
    }
...
export async function POST(request) {
    ...
    if (!workspaceSlug) {
        return NextResponse.json({ error: 'workspaceSlug is required' }, { status: 400, headers: CORS_HEADERS });
    }
...
export async function GET(request) {
    ...
    const where = { workspaceId: authResult.workspaceId };
    if (status) {
        where.status = status;
    }
