...
const msgRateBuckets = new Map();
async function checkMsgRateLimit(key) {
    if (redis) {
        try {
            ...
            if (count === 1)
                await redis.pexpire(redisKey, MSG_RATE_LIMIT_WINDOW_MS * 2);
    ...
    if (current.count >= MSG_RATE_LIMIT_MAX)
        return { limited: true };
    current.count++;
    return { limited: false };
}
...
export async function POST(request, { params }) {
    ...
    if (sessionToken) {
        ...
        if (rate.limited) {
            return NextResponse.json({ error: 'Too many messages' }, { status: 429, headers: CORS_HEADERS });
        }
        const conversation = await prisma.conversation.findFirst({
        ...
        if (!conversation.workspace.aiEnabled ||
        ...
        const readable = new ReadableStream({
            async start(controller) {
                try {
                    ...
                    const updateData = { aiConfidence: confidence };
                    if (shouldEscalate) {
