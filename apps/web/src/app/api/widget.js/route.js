...
const WIDGET_DIST = path.resolve(process.cwd(), '../../packages/widget/dist/widget.js');
export async function OPTIONS() {
    return new NextResponse(null, { status: 204, headers: CORS_HEADERS });
}
export async function GET() {
    if (!fs.existsSync(WIDGET_DIST)) {
        return NextResponse.json({ error: 'Widget not built. Run pnpm --filter @helpnest/widget build.' }, { status: 503 });
    }
