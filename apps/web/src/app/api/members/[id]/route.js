...
async function resolveCallerMember(userId) {
    ...
    if ('deactivatedAt' in body) {
        if (deactivatedAt === null) {
            ...
            if (target.role === 'OWNER') {
                const ownerCount = await countActiveOwners(target.workspaceId);
                if (ownerCount <= 1) {
                    return NextResponse.json({ error: 'Cannot deactivate the last owner of the workspace' }, { status: 400 });
                }
            }
