'use client';
import { getAllFontPresetUrls } from '@/lib/branding';
import { useRouter } from 'next/navigation';
import { useEffect, useState } from 'react';
import { AIThemeGenerator } from './AIThemeGenerator';
export function ThemePicker({ themes, fontPresets, radiusOptions, currentThemeId, currentFontPresetId, currentCustomCreamColor, currentCustomInkColor, currentCustomMutedColor, currentCustomBorderColor, currentCustomAccentColor, currentCustomGreenColor, currentCustomWhiteColor, currentCustomRadius, currentCustomHeadingFontFamily, currentCustomHeadingFontUrl, currentCustomBodyFontFamily, currentCustomBodyFontUrl, workspaceSlug, demoMode = false, }) {
    ...
    useEffect(() => {
        for (const trimmedUrl of [customHeadingFontUrl, customBodyFontUrl].map((url) => url.trim())) {
            if (trimmedUrl.length === 0)
                continue;
            const existing = Array.from(document.querySelectorAll('link[rel="stylesheet"]')).find((link) => link.href === trimmedUrl);
            if (existing)
                continue;
            const link = document.createElement('link');
            link.rel = 'stylesheet';
    ...
    const isDirty = selectedThemeId !== currentThemeId ||
    ...
    const visibleFontPresets = fontPresets.filter((p) => JSON.stringify(p.fonts) !== JSON.stringify(selectedTheme.fonts));
    const previewColors = {
    ...
    return (<div>
      <AIThemeGenerator onApply={(generated) => {
            setSaved(false);
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {themes.map((theme) => (<button key={theme.id} onClick={() => {
                setSaved(false);
                setSelectedThemeId(theme.id);
            }} className={`text-left rounded-xl border-2 overflow-hidden transition-all ${selectedThemeId === theme.id
                ? 'border-accent shadow-sm'
                : 'border-border hover:border-muted'}`}>
            {/* Color swatch strip */}
            <div className="h-1.5 flex">
              {Object.values(theme.colors).map((color, i) => (<div key={i} className="flex-1" style={{ backgroundColor: color }}/>))}
            </div>

            {/* Mini mockup */}
            <div className="p-2" style={{ backgroundColor: theme.colors.cream }}>
              <div className="rounded overflow-hidden border text-[10px]" style={{ borderColor: theme.colors.border }}>
                <div className="px-2 py-1 font-medium" style={{ backgroundColor: theme.colors.ink, color: theme.colors.cream }}>
                  {workspaceSlug} help
                </div>
                <div className="px-2 py-1.5 grid grid-cols-3 gap-1" style={{ backgroundColor: theme.colors.cream }}>
                  {[1, 2, 3].map((i) => (<div key={i} className="rounded p-1 border" style={{ backgroundColor: theme.colors.white, borderColor: theme.colors.border }}>
            ...
            <div className="px-3 py-2 bg-white flex items-center justify-between">
              ...
              {theme.dark && (<span className="text-[10px] bg-ink text-cream px-1.5 py-0.5 rounded-full">Dark</span>)}
            </div>
          </button>))}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
        ...
        }} className={`text-left rounded-xl border-2 overflow-hidden transition-all ${selectedFontPresetId.length === 0
            ? 'border-accent shadow-sm'
            : 'border-border hover:border-muted'}`}>
        ...
        {visibleFontPresets.map((preset) => (<button key={preset.id} onClick={() => {
                setSaved(false);
                setSelectedFontPresetId(preset.id);
            }} className={`text-left rounded-xl border-2 overflow-hidden transition-all ${selectedFontPresetId === preset.id
                ? 'border-accent shadow-sm'
                : 'border-border hover:border-muted'}`}>
            <div className="border-b border-border bg-cream px-4 py-3">
      ...
      <div className="rounded-xl border border-border bg-cream/50 p-4 mb-4">
        <div className="flex items-start justify-between gap-4 mb-3">
          ...
          {(customCreamColor.length > 0 ||
            ...
            customRadius.length > 0) && (<button onClick={() => {
                setSaved(false);
                setCustomCreamColor('');
                setCustomInkColor('');
                setCustomMutedColor('');
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {colorFields.map((field) => (<div key={field.label}>
            ...
            }} placeholder={field.placeholder} className="w-full rounded-lg border border-border bg-white px-3 py-2 text-sm text-ink placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-accent"/>
                <span className="h-9 w-9 shrink-0 rounded-lg border border-border bg-white" style={{ backgroundColor: (field.value.trim() || field.placeholder) }}/>
              </div>
            </div>))}
          <div>
            <label className="block text-xs font-medium text-ink mb-1">Corner radius</label>
            <select value={customRadius} onChange={(event) => {
            setSaved(false);
            setCustomRadius(event.target.value);
              ...
              {radiusOptions.map((option) => (<option key={option} value={option}>
                  {option.toUpperCase()}
                </option>))}
            </select>
          </div>
        ...
        <div className="mt-3 rounded-xl border border-border bg-white p-4">
          ...
          <div className="overflow-hidden border" style={{ backgroundColor: previewColors.cream, borderColor: previewColors.border, borderRadius: previewRadiusClass }}>
            <div className="px-4 py-3" style={{ backgroundColor: previewColors.ink, color: previewColors.cream }}>
              <p className="text-sm font-medium">{workspaceSlug} help</p>
            </div>
            <div className="grid gap-3 p-4 sm:grid-cols-2">
              <div className="border p-3" style={{ backgroundColor: previewColors.white, borderColor: previewColors.border, borderRadius: previewRadiusClass }}>
              ...
              <div className="p-3" style={{ backgroundColor: previewColors.green, color: previewColors.white, borderRadius: previewRadiusClass }}>
            ...
            <div className="px-4 pb-4">
              <button type="button" className="px-3 py-2 text-sm font-medium" style={{ backgroundColor: previewColors.accent, color: previewColors.white, borderRadius: previewRadiusClass }}>
                Accent action
              </button>
            </div>
          </div>
      ...
      <div className="rounded-xl border border-border bg-cream/50 p-4 mb-4">
        <div className="flex items-start justify-between gap-4 mb-3">
          ...
          {(customHeadingFontFamily.length > 0 ||
            ...
            customBodyFontUrl.length > 0) && (<button onClick={() => {
                setSaved(false);
        ...
        <div className="grid gap-3 sm:grid-cols-2 mt-3">
          <div>
            ...
            <input value={customHeadingFontFamily} onChange={(event) => {
            setSaved(false);
            setCustomHeadingFontFamily(event.target.value);
        }} placeholder={'e.g. "Sohne", "Helvetica Neue", sans-serif'} className="w-full rounded-lg border border-border bg-white px-3 py-2 text-sm text-ink placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-accent"/>
          </div>
          <div>
            <label className="block text-xs font-medium text-ink mb-1">Heading stylesheet URL</label>
            <input value={customHeadingFontUrl} onChange={(event) => {
            setSaved(false);
            setCustomHeadingFontUrl(event.target.value);
        }} placeholder="https://fonts.googleapis.com/css2?family=Heading+Font" className="w-full rounded-lg border border-border bg-white px-3 py-2 text-sm text-ink placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-accent"/>
          </div>
          <div>
            <label className="block text-xs font-medium text-ink mb-1">Body font family</label>
            <input value={customBodyFontFamily} onChange={(event) => {
            setSaved(false);
            setCustomBodyFontFamily(event.target.value);
        }} placeholder={'e.g. "Inter", system-ui, sans-serif'} className="w-full rounded-lg border border-border bg-white px-3 py-2 text-sm text-ink placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-accent"/>
          </div>
          <div>
            <label className="block text-xs font-medium text-ink mb-1">Body stylesheet URL</label>
            <input value={customBodyFontUrl} onChange={(event) => {
            setSaved(false);
            setCustomBodyFontUrl(event.target.value);
        }} placeholder="https://fonts.googleapis.com/css2?family=Body+Font" className="w-full rounded-lg border border-border bg-white px-3 py-2 text-sm text-ink placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-accent"/>
          </div>
        </div>

        {(customHeadingFontFamily.trim().length > 0 ||
            customBodyFontFamily.trim().length > 0) && (<div className="mt-3 rounded-xl border border-border bg-white px-4 py-3">
            <p className="text-xs text-muted mb-1">Custom font preview</p>
            <p className="text-xl text-ink" style={{ fontFamily: customHeadingFontFamily || undefined }}>
              {workspaceSlug} help center
            </p>
            <p className="text-sm text-muted mt-1" style={{ fontFamily: customBodyFontFamily || undefined }}>
              Search, articles, navigation, and AI responses use the body font.
            </p>
          </div>)}
      </div>

      {demoMode ? (<p className="text-sm text-muted">Branding changes cannot be saved in demo mode.</p>) : (<div className="flex items-center gap-3">
          <button onClick={save} disabled={saving || !isDirty} className="bg-ink text-cream px-4 py-2 rounded-lg text-sm hover:bg-ink/90 transition-colors disabled:opacity-50">
