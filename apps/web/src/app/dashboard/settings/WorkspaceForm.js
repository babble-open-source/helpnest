...
function looksLikeFaviconAsset(value) {
    const lower = value.toLowerCase();
    return (lower.includes('.ico') ||
        lower.includes('favicon') ||
        lower.includes('apple-touch-icon') ||
        lower.includes('mask-icon'));
}
export function WorkspaceForm({ name, slug, customDomain, logo, brandText, customBrandFontFamily, customBrandFontUrl, favicon, metaTitle, metaDescription, appUrl, demoMode = false, }) {
    ...
    const logoMatchesFavicon = normalizedLogo.length > 0 &&
    ...
    useEffect(() => {
        ...
        if (trimmedBrandFontUrl.length === 0)
            return;
        const existing = Array.from(document.querySelectorAll('link[rel="stylesheet"]')).find((link) => link.href === trimmedBrandFontUrl);
        if (existing)
            return;
    ...
    return (<div className="space-y-4">
      {demoMode && (<p className="text-sm text-muted">Workspace settings cannot be changed in demo mode.</p>)}
      <div>
        <label className="block text-sm font-medium text-ink mb-1">Name</label>
        <input value={values.name} onChange={set('name')} disabled={demoMode} className={inputClass}/>
      </div>
      <div>
        ...
        <div className={`flex items-center border border-border rounded-lg overflow-hidden${demoMode ? ' opacity-60' : ''}`}>
          ...
          <input value={values.slug} onChange={set('slug')} readOnly={demoMode} className={`flex-1 px-3 py-2 text-sm bg-white text-ink focus:outline-none${demoMode ? ' cursor-not-allowed select-none' : ''}`}/>
        </div>
      </div>
      <div>
        <label className="block text-sm font-medium text-ink mb-1">Custom Domain</label>
        <input value={values.customDomain} onChange={set('customDomain')} placeholder="support.yourcompany.com" disabled={demoMode} className={inputClass + ' placeholder:text-muted'}/>
        ...
        {values.customDomain.trim().length > 0 && (<div className="mt-2 rounded-lg border border-border bg-cream px-3 py-2 text-xs text-muted space-y-0.5">
      ...
      <div>
        ...
        <input value={values.logo} onChange={set('logo')} placeholder="https://cdn.yourcompany.com/logo.svg" disabled={demoMode} className={inputClass + ' placeholder:text-muted'}/>
        <p className="mt-1 text-xs text-muted">
        ...
        {logoWarning && (<div className="mt-3 rounded-xl border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-900">
            {logoWarning}
          </div>)}
        {values.logo.trim().length > 0 && (<div className="mt-3 inline-flex items-center gap-3 rounded-xl border border-border bg-cream px-3 py-2">
            <div className="flex h-12 min-w-[3rem] max-w-[12rem] items-center justify-start overflow-hidden rounded-lg border border-border bg-white px-2 py-1.5">
              <img src={values.logo.trim()} alt={`${values.name} logo preview`} className="block h-full w-auto max-w-full object-contain object-left"/>
            </div>
            <div>
      ...
      <div>
        ...
        <input value={values.brandText} onChange={set('brandText')} placeholder={values.name || 'Acme'} disabled={demoMode} className={inputClass + ' placeholder:text-muted'}/>
        <p className="mt-1 text-xs text-muted">
      ...
      <div>
        ...
        <input value={values.customBrandFontFamily} onChange={set('customBrandFontFamily')} placeholder="Sohne, DM Sans, Avenir Next" disabled={demoMode} className={inputClass + ' placeholder:text-muted'}/>
        <p className="mt-1 text-xs text-muted">
      ...
      <div>
        ...
        <input value={values.customBrandFontUrl} onChange={set('customBrandFontUrl')} placeholder="https://fonts.googleapis.com/css2?family=Your+Brand+Font" disabled={demoMode} className={inputClass + ' placeholder:text-muted'}/>
        <p className="mt-1 text-xs text-muted">
          Optional. Required only when the brand font is not already available on the page.
        </p>
        {(values.brandText.trim().length > 0 || values.customBrandFontFamily.trim().length > 0) && (<div className="mt-3 inline-flex items-center gap-3 rounded-xl border border-border bg-cream px-3 py-2">
            <div className="rounded-lg border border-border bg-white px-4 py-2">
              <span className="block text-2xl text-ink" style={values.customBrandFontFamily.trim().length > 0
      ...
      <div>
        ...
        <input value={values.favicon} onChange={set('favicon')} placeholder="https://cdn.yourcompany.com/favicon.png" disabled={demoMode} className={inputClass + ' placeholder:text-muted'}/>
        <p className="mt-1 text-xs text-muted">
          Used for the browser tab icon. Falls back to your company logo if empty.
        </p>
        {values.favicon.trim().length > 0 && (<div className="mt-3 inline-flex items-center gap-3 rounded-xl border border-border bg-cream px-3 py-2">
            <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-lg border border-border bg-white p-1">
              <img src={values.favicon.trim()} alt={`${values.name} favicon preview`} className="h-full w-full object-contain"/>
            </div>
            <div>
      ...
      <div>
        ...
        <input value={values.metaTitle} onChange={set('metaTitle')} placeholder={`${values.name} Help Center`} disabled={demoMode} className={inputClass + ' placeholder:text-muted'}/>
      ...
      <div>
        ...
        <textarea value={values.metaDescription} onChange={set('metaDescription')} placeholder={`Support docs, guides, and answers for ${values.name}.`} rows={4} disabled={demoMode} className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-white text-ink placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-accent resize-y disabled:opacity-50 disabled:cursor-not-allowed"/>
        <p className="mt-1 text-xs text-muted">
      ...
      {!demoMode && (<div className="flex items-center gap-3 pt-1">
          <button onClick={save} disabled={status === 'saving'} className="bg-ink text-cream px-4 py-2 rounded-lg text-sm hover:bg-ink/90 transition-colors disabled:opacity-50">
      ...
      <div className="pt-3 border-t border-border">
        ...
        <a href={`${appUrl}/${values.slug}/help`} target="_blank" rel="noopener noreferrer" className="text-sm font-mono text-accent hover:underline break-all">
