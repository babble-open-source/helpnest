...
function helpfulRate(helpful, notHelpful) {
...
export default async function DashboardPage() {
    ...
    const feedbackCount30DaysPromise = feedbackPrisma.articleFeedback.count({
        where: {
            workspaceId: member.workspaceId,
            createdAt: { gte: last30Days },
        },
    });
    const totalFeedbackCountPromise = feedbackPrisma.articleFeedback.count({
        where: { workspaceId: member.workspaceId },
    });
    ...
    return (<div className="p-4 sm:p-8">
      ...
      {/* AI Support Stats */}
      {totalConversations > 0 && (<div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-10">
          {[
            ...
            ].map((s) => (<div key={s.label} className="bg-white rounded-xl border border-border p-5">
      ...
      {(aiNewDraftCount > 0 || aiUpdateCount > 0) && (<div className="flex flex-wrap gap-4 mb-10">
          {aiNewDraftCount > 0 && (<a href="/dashboard/articles?filter=ai-drafts" className="bg-white rounded-xl border border-border p-5 hover:border-accent transition-colors">
              <p className="text-2xl font-semibold text-ink">{aiNewDraftCount}</p>
              <p className="text-sm text-muted mt-1">AI Drafts to Review</p>
            </a>)}
          {aiUpdateCount > 0 && (<a href="/dashboard/articles?filter=ai-updates" className="bg-white rounded-xl border border-border p-5 hover:border-accent transition-colors">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
        {stats.map((stat) => (<div key={stat.label} className="bg-white rounded-xl border border-border p-5">
      ...
      <div className="mb-10">
        ...
        <div className="bg-white rounded-xl border border-border overflow-hidden">
          {needsAttention.length === 0 ? (<div className="p-6">
              ...
              {needsAttention.map((article) => (<div key={article.id} className="flex items-center gap-4 p-4">
      ...
      <div>
        ...
        <div className="bg-white rounded-xl border border-border divide-y divide-border">
          {recentArticles.map((article) => (<div key={article.id} className="flex items-center gap-4 p-4">
              <div className="flex-1 min-w-0">
                <p className="font-medium text-ink truncate">{article.title}</p>
                <p className="text-sm text-muted mt-0.5">{article.collection.title}</p>
              </div>
              <span className={`text-xs px-2 py-0.5 rounded-full ${article.status === 'PUBLISHED'
