...
import { hasWorkspaceBrandTextColumn, hasWorkspaceCustomAccentColorColumn, hasWorkspaceCustomBrandFontFamilyColumn, hasWorkspaceCustomBrandFontUrlColumn, hasWorkspaceCustomBodyFontFamilyColumn, hasWorkspaceCustomBodyFontUrlColumn, hasWorkspaceCustomBorderColorColumn, hasWorkspaceCustomCreamColorColumn, hasWorkspaceCustomGreenColorColumn, hasWorkspaceCustomHeadingFontFamilyColumn, hasWorkspaceCustomHeadingFontUrlColumn, hasWorkspaceCustomInkColorColumn, hasWorkspaceCustomMutedColorColumn, hasWorkspaceCustomRadiusColumn, hasWorkspaceCustomWhiteColorColumn, hasWorkspaceFontPresetColumn, prisma, } from '@/lib/db';
...
export default async function DashboardLayout({ children }) {
    ...
    const showDefaultPasswordBanner = !demoMode &&
    ...
    const [fontPresetRecord, brandTextRecord, customCreamColorRecord, customInkColorRecord, customMutedColorRecord, customBorderColorRecord, customAccentColorRecord, customGreenColorRecord, customWhiteColorRecord, customRadiusRecord, customHeadingFontFamilyRecord, customHeadingFontUrlRecord, customBodyFontFamilyRecord, customBodyFontUrlRecord, customBrandFontFamilyRecord, customBrandFontUrlRecord,] = await Promise.all([
    ...
    const overrides = {
        ...
        customRadius: customRadiusRecord?.customRadius ?? null,
    ...
    return (<>
      ...
      <div className="h-screen bg-cream flex flex-col overflow-hidden">
        {demoMode && (<div className="fixed top-4 right-4 z-50 flex items-center gap-2">
            <a href="https://github.com/babble-open-source/helpnest" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 bg-white border border-border text-ink text-xs font-medium px-2.5 py-1 rounded-full shadow hover:bg-cream transition-colors">
        ...
        <DashboardSidebar workspaceId={member.workspaceId} workspaceName={workspace.name} workspaceLogo={workspace.logo} workspaceBrandText={brandTextRecord?.brandText ?? null} userName={session.user.name ?? 'User'} userEmail={session.user.email ?? ''} userInitial={userInitial}/>
