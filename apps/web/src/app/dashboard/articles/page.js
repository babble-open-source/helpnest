...
function feedbackSummary(helpful, notHelpful) {
...
export default async function ArticlesPage(props) {
    ...
    const where = {
        ...
        ...(aiDraftsFilter
            ...
            : aiUpdatesFilter
                ? { aiGenerated: true, status: 'PUBLISHED', NOT: { draftContent: null } }
                : searchParams.status
    ...
    return (<div className="p-4 sm:p-8">
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        ...
        <Link href="/dashboard/articles/new" className="bg-ink text-cream px-3 sm:px-4 py-2 rounded-lg text-sm hover:bg-ink/90 transition-colors font-medium shrink-0">
          + New Article
        </Link>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        ].map((tab) => (<Link key={tab.label} href={tab.href} className={`px-3 py-1.5 rounded-lg text-sm transition-colors ${tab.active ? 'bg-ink text-cream' : 'text-muted hover:text-ink hover:bg-cream'}`}>
            {tab.label}
          </Link>))}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <form className="flex items-center gap-2 bg-white border border-border rounded-lg px-3 py-2 w-full sm:w-auto">
          ...
          <input name="q" defaultValue={searchParams.q} placeholder="Search articles..." className="text-sm outline-none text-ink placeholder:text-muted bg-transparent flex-1 sm:w-48"/>
        </form>

        <div className="flex items-center gap-2 flex-wrap">
          {['', 'DRAFT', 'PUBLISHED', 'ARCHIVED'].map((s) => (<Link key={s} href={`/dashboard/articles${s ? `?status=${s}` : ''}`} className={`px-3 py-1.5 rounded-lg text-sm transition-colors ${(searchParams.status ?? '') === s
                ? 'bg-ink text-cream'
      ...
      {/* Table */}
      {articles.length === 0 ? (<div className="bg-white rounded-xl border border-border p-12 text-center">
        ...
        </div>) : (<div className="bg-white rounded-xl border border-border overflow-hidden">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-border">
              {articles.map((article) => (<tr key={article.id} className="hover:bg-cream/30 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <p className="font-medium text-ink text-sm truncate max-w-xs">
                        {article.title}
                      </p>
                      {article.aiGenerated && article.status === 'DRAFT' && (<span className="shrink-0 text-xs px-1.5 py-0.5 rounded bg-accent/10 text-accent font-medium">
                          AI
                        </span>)}
                    </div>
                    {article.excerpt && (<p className="text-xs text-muted mt-0.5 truncate max-w-xs">
                        {article.excerpt}
                      </p>)}
                  </td>
                  ...
                  <td className="px-4 py-3 text-right hidden lg:table-cell">
                    ...
                    const rateTone = (summary.helpfulRate ?? 0) >= 80
                        ? 'text-green'
                  ...
                  <td className="px-4 py-3 text-right hidden lg:table-cell">
                    <span className="text-sm text-muted">
                      {new Intl.RelativeTimeFormat('en', { numeric: 'auto' }).format(Math.round((article.updatedAt.getTime() - nowMs()) /
                    (1000 * 60 * 60 * 24)), 'day')}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <ArticleActions articleId={article.id} articleTitle={article.title} articleStatus={article.status} demoMode={demoMode} isSeeded={article.isSeeded}/>
                  </td>
                </tr>))}
            </tbody>
          </table>
