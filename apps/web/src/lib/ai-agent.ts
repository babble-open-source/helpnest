  ...
  if (queryEmbedding.length > 0) {
    try {
      ...
      if (articleIds.length > 0) {
        ...
        return articleIds.map((id) => byId.get(id)).filter((r): r is ArticleRow => r !== undefined)
      }
    } catch {
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
    ...
    for (const tc of toolCalls) {
      if (tc.name === 'search_articles') {
        ...
        toolResults.push(
          `[report_confidence result]: Confidence recorded at ${confidence.toFixed(2)}. ${reasoning}`,
        )
