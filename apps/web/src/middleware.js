...
const APP_ORIGIN = process.env.NEXT_PUBLIC_APP_URL?.replace(/\/$/, '') ??
    ...
    '';
function rewriteToHelp(req, slug) {
    ...
    if (pathname === '/widget.js')
        return null;
    if (pathname.startsWith('/dashboard') || pathname.startsWith('/login') || pathname.startsWith('/invite/'))
        return null;
...
function handleCustomDomain(req) {
    if (!CUSTOM_DOMAIN || !CUSTOM_DOMAIN_SLUG)
        return null;
    const host = (req.headers.get('host') ?? '').split(':')[0] ?? '';
 ...
 */
async function handleDBCustomDomain(req) {
    if (!APP_ORIGIN)
        return null;
    ...
    if (host.endsWith(`.${HELP_CENTER_DOMAIN}`) || host === CUSTOM_DOMAIN)
        return null;
    ...
    try {
        ...
        const res = await fetch(`${APP_ORIGIN}/api/internal/resolve-domain?host=${encodeURIComponent(host)}`, { signal: controller.signal });
