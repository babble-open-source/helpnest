 ...
 */
function groupListItems(text) {
    const lines = text.split('\n');
    const result = [];
    let mode = null;
    let buffer = [];
    let olStart = 1; // start attribute for the next <ol> segment
    function countOlItemsInBuffer() {
...
function renderMarkdown(md) {
    ...
    let out = md.replace(/```(\w*)\n?([\s\S]*?)```/g, (_match, _lang, code) => {
        codeBlocks.push(`<pre class="bg-ink text-cream rounded-lg p-4 overflow-x-auto my-5"><code class="text-sm font-mono">${escapeHtml(code.trim())}</code></pre>`);
    ...
    out = out.replace(/^\|(.+)\|\n\|[-| :]+\|\n((?:\|.+\|\n?)+)/gm, (match) => {
        ...
        return (`<div class="overflow-x-auto my-5">` +
 ...
 */
export function fixOrderedListCounters(html) {
...
function wrapTiptapTables(html) {
...
export function ArticleContent({ content }) {
    ...
    return (<div className="article-prose hn-prose" dangerouslySetInnerHTML={{ __html: html }}/>);
}
