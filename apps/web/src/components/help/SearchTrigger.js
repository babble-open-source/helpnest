...
export function SearchTrigger({ workspace }) {
    function handleClick() {
        window.dispatchEvent(new CustomEvent('helpnest:search:open'));
    }
    return (<>
      <button onClick={handleClick} className="flex items-center gap-3 bg-white/10 hover:bg-white/20 transition-colors rounded-xl px-5 py-3 max-w-lg mx-auto w-full text-left">
