...
export function EditorOutline({ editor }: { editor: Editor }) {
  ...
  return (
    <aside className="w-56 bg-white border-r border-border flex flex-col shrink-0 overflow-y-auto">
      ...
      {headings.length === 0 ? (
        <p className="p-4 text-xs text-muted/60 text-center leading-relaxed">
        ...
        <nav className="p-2 space-y-0.5">
          {headings.map((h, i) => (
            <button
              ...
              className={`w-full text-left rounded px-2 py-1 hover:bg-cream text-ink/70 hover:text-ink transition-colors truncate ${
                h.level === 1
                  ? 'text-sm font-medium'
                  : h.level === 2
                  ? 'text-xs pl-4'
